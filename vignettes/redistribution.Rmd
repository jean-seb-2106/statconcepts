...
#Autre fonction avec une une augmentation des inégalités un peu plus élaborée

# x <- grandile[,"REV_DISPONIBLE"]
# varnum <- sort(x)
# pct_cumx <- cumsum(x)/sum(x) 
# tx <- 100
# #Création d'une base avec les plus pauvres 
...
#          )
# varnum_apresmodif <- c(y$varnum_apres,z$varnum_apres)
# plot(Lc(varnum_apresmodif),col="darkred",lwd=5)
# 
...
x <- modifier_repartition(grandile,"REV_DISPONIBLE",dimin = FALSE,tx=99)
...
```{r}
x <- modifier_repartition(grandile,"REV_DISPONIBLE",dimin = FALSE,tx=50)
